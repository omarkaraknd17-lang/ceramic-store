const state = { products: [], filtered: [], request: JSON.parse(localStorage.getItem("ceramic-request") || "[]") };
...
function openProduct(product) {
  $("#dialogContent").innerHTML = `<div class="dialog-layout"><button class="dialog-tile image-zoom-button" type="button" aria-label="הגדלת תמונת המוצר"><img src="${productImage(product)}" alt="תמונת קטלוג של ${product.sku_model_code}" /><span>לחצו להגדלה</span></button><div class="dialog-info"><p class="eyebrow">${label(product.brand)}</p><h2>${label(product.product_name, product.collection_series)}</h2><dl><dt>קוד</dt><dd><code>${product.sku_model_code}</code></dd><dt>קטגוריה</dt><dd>${label(product.product_type_category)}</dd><dt>מידה</dt><dd>${(product.sizes_mm || []).join(", ") || "באימות"}</dd><dt>גימור</dt><dd>${label(product.finish)}</dd><dt>עיצוב / גוון</dt><dd>${label(product.color_design)}</dd></dl><button class="button button-primary add-dialog-product" type="button">הוספה לבקשת הצעת מחיר</button><p class="quote-note">התמונה היא חיתוך מהמוצר המקורי בקטלוג. המחיר והמלאי מאושרים לפני הזמנה.</p></div></div>`;
...
function renderRequest() {
  ...
  state.request.forEach((item, index) => {
    ...
    row.innerHTML = `<div><strong>${label(item.product.product_name, item.product.collection_series)}</strong><small>${item.sku} · ${(item.product.sizes_mm || []).join(", ")}</small></div><input type="text" inputmode="decimal" aria-label="כמות עבור ${item.sku}" placeholder="כמות" value="${item.quantity}"><button class="remove-item" type="button">הסרה</button>`;
...
fetch("../data/master-products.json").then(response => { if (!response.ok) throw new Error("Catalog data could not load"); return response.json(); }).then(products => { state.products = products.filter(p => p.sku_model_code); state.filtered = [...state.products]; $("#productCount").textContent = state.products.length; populateFilters(state.products); bindEvents(); saveRequest(); render(); }).catch(() => { grid.innerHTML = "<p class='empty-state'>לא ניתן לטעון את הקטלוג. יש להפעיל את האתר דרך שרת מקומי.</p>"; });
