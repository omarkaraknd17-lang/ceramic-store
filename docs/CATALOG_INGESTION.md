...
`data/collection-index.json` is a middle layer for collection facts that are visually readable (for example, a series name and its declared sizes) but whose complete item-level variant list is not yet verified. It is also not a public price list.
